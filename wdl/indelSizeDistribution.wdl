...
task indelDistribution_WholeExome_HappyResults {
  ...
  command <<<
    ...
    end=$((end - 1)); bcftools stats ~{truthWholeExomeVCF}| sed -n "$start,$end p" > ~{indelDistribution_WholeExome}
  >>>
...
task indelSizeDistribution_CodingExons_HappyResults {
  ...
  command <<<
            Rscript ~{Rscript_indelSize} ~{indelDistribution_CodingExons} ~{codingExons_annotated_TPonly_vcf_gz} ~{codingExons_annotated_FPonly_vcf_gz} ~{codingExons_annotated_FNonly_vcf_gz} ~{indelSizeDistribution_CodingExons} ~{indelSizeDistributionPlot_CodingExons}
  >>>
  runtime {
